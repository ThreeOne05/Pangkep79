...
export default function MinumanLayout() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Perbaikan: akses array dari data.products, bukan data langsung!
        const produkArray = Array.isArray(data.products) ? data.products : [];
        const minumanCards = produkArray
  ...
  // Handler untuk product baru dari form
        ...
        <div className="px-20">
          {cards.length > 0 ? (
            ...
            <div className="text-center py-10">
              <p>Tidak ada produk minuman tersedia</p>
            </div>
          )}
        </div>
        {isFormOpen && (
